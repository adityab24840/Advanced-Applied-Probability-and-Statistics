...
p1 = ggplot(data = heptathlon) +
  geom_point(aes(x = run200m, y = longjump), size = 2)+   
...
p2 = ggplot(data = heptathlon) +
  geom_point(aes(x = run200m, y = longjump, color = sprint), size = 2)+   
  labs(x = 'run200m (seconds)', y = 'longjump (m)') +
   theme(axis.text = element_text(size = 12),
   axis.text.x = element_text(size = 14),
   axis.text.y = element_text(size = 14),
...
cor(heptathlon$longjump, heptathlon$run200m)
```
